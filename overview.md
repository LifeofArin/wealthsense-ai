...
**3. Transformer**
A newer architecture that reads the full 30-day window at once instead of one day at a time. It learns which days matter most for the next-day forecast. A causal mask stops the model from using future information.
...
**Q: Why not use a general language model for predictions?**
A: General language models are not built for numerical time series. These models are neural networks trained on financial data with proper train, validation, and test splits.
